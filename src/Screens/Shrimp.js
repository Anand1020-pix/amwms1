...
export default function Shrimp({ navigation }) {
  ...
  const handleItemPress = async (item) => {
    ...
    try {
      // Fetch the latest 5 values from the endpoint
      ...
      const latestValues = latestData.slice(0, 5).map(data => parseFloat(data.payload[item.title]) || 0);
      const latestTimestamps = latestData.slice(0, 5).map(data => graphtime(parseFloat(data.timestamp)));
  ...
  const formatTimestamp = (timestamp) => {
    ...
    const options = { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', fractionalSecondDigits: 3, hour12: true, timeZone: 'UTC' };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
       ...
       <Text></Text>
       <Text></Text>
      <Text style={styles.headerText}>Water Quality Performance(Shrimp)</Text>
      <FlatList
        data={data}
